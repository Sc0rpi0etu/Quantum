...
public class TankUltiRun extends BukkitRunnable {
	private JoinQuitListener joinQuitListener;
	
	public TankUltiRun(JoinQuitListener joinQuitListener) {
	...
	public void run() {
		for (Player player : joinQuitListener.playerConnected) {
			if (RoleMethodsFunctions.getWarrior(player.getUniqueId().toString()) != null) {
				if (RoleMethodsFunctions.getTank(player.getUniqueId().toString()).getUltiTime() > 0) {
					RoleMethodsFunctions.getTank(player.getUniqueId().toString()).setUltiTime(RoleMethodsFunctions.getTank(player.getUniqueId().toString()).getUltiTime() - 1);
				}
			}
		}
	}
